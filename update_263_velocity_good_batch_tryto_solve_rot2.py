...
# import numpy as np
#
# def compute_rot_vel_and_scaling(positions, standard_velocity_abs_mean=0.015, epsilon=1e-8):
#     seq_len = positions.shape[0]
#
#     # 计算所有位置的变化
#     delta_positions = positions[1:] - positions[:-1]
#     delta_positions = torch.cat([delta_positions[0:1], delta_positions], dim=0)  # 重复第一个delta
#
#     # 计算相邻位置向量之间的点积和模长
#     dot_products = torch.sum(delta_positions[:-1] * delta_positions[1:], dim=1)  # 点积
#     norms = torch.norm(delta_positions, dim=1)  # 向量模长
#
#     # 防止模长为零，添加一个小的epsilon
#     norms = norms + epsilon
#
#     # 计算夹角余弦值
#     cos_theta = dot_products / (norms[:-1] * norms[1:])
#
#     # 将cos_theta限定在[-1, 1]范围内
...
#
#     # 确定旋转方向（正负号）
#     cross_products = delta_positions[:-1, 0] * delta_positions[1:, 1] - delta_positions[:-1, 1] * delta_positions[1:, 0]
#     sign = torch.sign(cross_products)
#     theta = theta * sign
#
#     # 将夹角赋值给旋转速度
...
#     print("standard_velocity_abs_mean", standard_velocity_abs_mean, "mean_abs_rot_vel", mean_abs_rot_vel.item())
#
#     # 计算标准的缩放因子
#     standard_scaling_factor = standard_velocity_abs_mean / mean_abs_rot_vel.item() if mean_abs_rot_vel != 0 else 1.0
#
...
#     output_directory = '/Users/huangziheng/PycharmProjects/final_LLM_enhance_v4/trajectory_guidance/263output_afterguidance_notan2/'
#
